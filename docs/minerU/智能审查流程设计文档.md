...
- `src/mastra/tools/extraction-item-storage-tool.ts`
- `src/lib/db/schema.ts` 中的 `extractionItems`
...
flowchart LR
  ...
  S --> T["tender-review-agent<br/>投标文件审查专家"]
  S --> R["report-generation-agent<br/>审查报告撰写专家"]
  S --> I["image-review-agent<br/>图像审查专家"]

  E --> DB1["extraction_items"]
  T --> R
  I --> R
  R --> DB2["review_reports / review_issues / review_item_results"]
```
...
`extraction-agent` 负责从招标文件中提取 5 类固定审查项，用于后续投标文件审查。

职责：

- 读取文档全文，识别章节结构。
- 提取 5 类固定审查项：完整性、关键信息一致性、质量目标、项目名称一致性、编制依据。
- 支持增量更新：已有同 title 项传入 id 覆盖，新项自动新增。
- 将结果写入 `extraction_items`。
...
`tender-review-agent` 负责基于审查项对投标文件进行逐条审查。

执行流程：

1. 调用 `resolve-review-report` 验证或创建报告 ID。
2. 调用 `get-review-items` 获取项目全部审查项。
3. 调用 `document-reader` 获取投标文件 blocks。
4. 对每个审查项逐一判断：
   - `pass`：满足要求，记录证据 block。
   - `fail`：存在问题，产出 issue 并关联 block。
   - `needs_manual_review`：证据不足或无法确认。
5. 调用 `structured-review-storage` 落库审查项结果和问题。

输出内容包括 reviewItemResults[]、issues[]，每个问题定位到页码、区块和文本片段。
...
`image-review-agent` 负责分析文档中的图片是否存在暗标风险。
...
输出格式：`{ hasRisk, riskType, riskText, confidence }`，只报告明确可见的风险，不做推测。
...
erDiagram
  ...
  documents ||--o{ extraction_items : extracts
  review_reports ||--o{ review_issues : contains
  review_reports ||--o{ review_item_results : evaluates
  extraction_items ||--o{ review_item_results : judged_by
...
| `extraction_items` | 从招标文件中提取的审查项（5类固定） |
...
| 提取结果查询 | `GET /api/documents/[documentId]/extract` | 查询审查项 |
...
`src/mastra/config/review.ts` 已实现，包含模型配置、supervisor 指令、tender-review-agent 指令、report-generation-agent 指令和 working memory 模板。当前涉及导入项：

- `reviewModelConfig`
- `supervisorInstructions`
- `tenderReviewInstructions`
- `reportGenerationInstructions`
- `supervisorWorkingMemoryTemplate`
- `reportWorkingMemoryTemplate`
- `reportGenerationInstructions`
配置文件已实现，后续可统一模型、提示词、评分原则和输出约束。
...
后续可考虑引入显式 workflow，将“读取文档、读取审查项、投标文件审查、报告保存”固化为可观测步骤。
...
1. 固化 supervisor 的最小必经步骤，避免审查流程漂移。
2. 完善报告落库失败时的错误提示和重试机制。
3. 将旧规则审查逻辑整理为可复用规则库。
