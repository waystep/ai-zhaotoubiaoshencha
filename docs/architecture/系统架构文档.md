...
智能招标审查平台 是一个 AI 驱动的招标文件智能审查与分析平台，旨在帮助招标机构和评审专家高效完成招标文件的合规性审查、智能评分与报告生成。
...
│  │              Mastra 多智能体系统                              │ │
│  │  ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────────┐ │ │
│  │  │ Supervisor │ │ Extraction │ │ Content   │ │ Report Gen   │ │ │
...
│   ├── content-review-agent.ts      # 内容审查专家
│   ├── image-review-agent.ts        # 图像审查专家
│   ├── tender-response-agent.ts     # 响应评估专家
│   ├── report-generation-agent.ts   # 报告生成专家
│   └ tender-review-agent.ts        # 旧版智能体 (兼容)
│
├── tools/                        # 15 个工具
│   ├── document-reader-tool.ts      # 文档读取 (分页)
│   ├── review-item-storage-tool.ts  # 审查项存储
│   ├── response-item-storage-tool.ts # 响应项存储
│   ├── get-review-items-tool.ts     # 获取审查项
│   ├── get-response-items-tool.ts   # 获取响应项
...
│   └ document-analysis-tool.ts     # 文档分析
│   ├── checkpoint-design-tool.ts    # 检查点设计
│   └ review-item-query-tool.ts     # 审查项查询
│   └ response-item-query-tool.ts   # 响应项查询
│   └ get-document-info-tool.ts     # 文档信息获取
│
├── prompts/                      # 提示词模板
└── workflows/                    # 工作流定义
```
...
| Agent | 职责 | 输入 | 输出 |
|-------|------|------|------|
| **tender-review-supervisor** | 总协调者，协调各专业智能体 | projectId, reportId, docType | 完成状态、issueCount、score、recommendation |
| **extraction-agent** | 从招标/法律文件提取审查项和响应项 | documentId | reviewItems[], responseItems[] |
| **content-review-agent** | 审查文本/表格 blocks 合规性 | documentId, reviewItems | issues[], reviewItemResults[] |
| **image-review-agent** | 审查图表/印章等图像内容 | documentId, blocks | issues[] |
| **tender-response-agent** | 评估响应项覆盖情况 | documentId, responseItems | responseItemResults[] |
| **report-generation-agent** | 汇总结果并结构化落库 | 各Agent结果 | report (completed), issues[], results[] |
| **tender-review-agent** | 旧版单智能体审查 (向后兼容) | projectId, documentId | report |
...
│                           ▼                                  │
│  ┌────────────────────────────────────────────────────────┐ │
│  │ Step 6: report-generation-agent 汇总并落库              │ │
│  │ - issues[]                                             │ │
│  │ - reviewItemResults[]                                  │ │
│  │ - responseItemResults[]                                │ │
...
│                           ▼                                  │
│  ┌────────────────────────────────────────────────────────┐ │
│  │ Step 7: 确认 report 状态更新为 completed                │ │
│  └────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```
